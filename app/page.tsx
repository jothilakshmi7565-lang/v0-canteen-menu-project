...
export default function Home() {
  ...
  const [userType, setUserType] = useState<"user" | "chef" | "admin" | null>(null)
  ...
  const handleRoleSelect = (role: "user" | "chef" | "admin") => {
    setUserType(role)
    setScreen("login")
  }
          ...
          {screen === "role" ? (
            <div>
              ...
              <Card className="p-8 border-0 shadow-2xl bg-white/95 backdrop-blur">
                <h2 className="text-3xl font-bold mb-2 text-center bg-linear-to-r from-orange-600 to-red-600 bg-clip-text text-transparent">
                  Select Your Role
                </h2>
                ...
                <div className="space-y-4">
                  <Button
                    ...
                    className="w-full bg-linear-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-bold py-6 text-lg rounded-xl transition transform hover:scale-105 hover:shadow-lg"
                  >
                    👤 Customer Login
                  </Button>
                  <Button
                    onClick={() => handleRoleSelect("chef")}
                    className="w-full bg-linear-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold py-6 text-lg rounded-xl transition transform hover:scale-105 hover:shadow-lg"
                  >
                    👨‍🍳 Canteen Staff Login
                  </Button>
                  <Button
                    onClick={() => handleRoleSelect("admin")}
                    className="w-full bg-linear-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white font-bold py-6 text-lg rounded-xl transition transform hover:scale-105 hover:shadow-lg"
                  >
                    🔐 Admin Login
                  </Button>
                </div>
            ...
            <>
              <div className="text-center mb-8 animate-fade-in">
                <h1 className="text-4xl font-black bg-linear-to-r from-orange-600 to-red-600 bg-clip-text text-transparent mb-2">
                  MEC Canteen
                </h1>
                <p className="text-sm text-orange-700 font-semibold mb-3">
                  {userType === "user" ? "👤 Customer Portal" : userType === "chef" ? "👨‍🍳 Canteen Staff Portal" : "🔐 Admin Portal"}
                </p>
                <button
